...
for TILE_SIZE in ts:
	for TILE_MATCH_RES in tmr:
		for ENLARGEMENT in en:
			timestamp_ = time.time()
			TILE_BLOCK_SIZE = TILE_SIZE / max(min(TILE_MATCH_RES, TILE_SIZE), 1)
			WORKER_COUNT = max(cpu_count() - 1, 1)
			OUT_FILE = 'mosaic_' + str(int(TILE_SIZE)) + '_' + str(int(TILE_MATCH_RES)) + '_' + str(int(ENLARGEMENT)) + '_' + str(int(timestamp_)) + '.jpeg'
			EOQ_VALUE = None
			print("TILE_SIZE IS: " + str(TILE_SIZE))
			print("TILE_MATCH_RES IS: " + str(TILE_MATCH_RES))
			print("ENLARGEMENT IS: " + str(ENLARGEMENT))
